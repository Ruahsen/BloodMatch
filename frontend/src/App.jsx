      ...
      <header className="topbar">
        <div className="topbar-inner">
          ...
          <nav className="nav" aria-label="Main Navigation">
            <Link to="/" className={`nav-link ${isActive('/') ? 'active' : ''}`} aria-current={isActive('/') ? 'page' : undefined}>
              Home
            </Link>

            {user ? (
              <>
                <Link to="/profile" className={`nav-link ${isActive('/profile') ? 'active' : ''}`} aria-current={isActive('/profile') ? 'page' : undefined}>
                  Profile
                </Link>
                <Link to="/requests/mine" className={`nav-link ${isActive('/requests/mine') ? 'active' : ''}`} aria-current={isActive('/requests/mine') ? 'page' : undefined}>
                  My Requests
                </Link>

                {user.role === 'officer' && (
                  <>
                    <Link to="/officer/dashboard" className={`nav-link ${isActive('/officer/dashboard') ? 'active' : ''}`} aria-current={isActive('/officer/dashboard') ? 'page' : undefined}>
                      Dashboard
                    </Link>
                    <Link to="/demand-map" className={`nav-link ${isActive('/demand-map') ? 'active' : ''}`} aria-current={isActive('/demand-map') ? 'page' : undefined}>
                      Demand Map
                    </Link>
                    <Link to="/analytics" className={`nav-link ${isActive('/analytics') ? 'active' : ''}`} aria-current={isActive('/analytics') ? 'page' : undefined}>
                      Analytics
                    </Link>
                    <Link to="/officer/verifications" className={`nav-link ${isActive('/officer/verifications') ? 'active' : ''}`} aria-current={isActive('/officer/verifications') ? 'page' : undefined}>
                      Verifications
                    </Link>
                    <Link to="/officer/confirmations" className={`nav-link ${isActive('/officer/confirmations') ? 'active' : ''}`} aria-current={isActive('/officer/confirmations') ? 'page' : undefined}>
                      Confirmations
                    </Link>
                    <Link to="/officer/audit-logs" className={`nav-link ${isActive('/officer/audit-logs') ? 'active' : ''}`} aria-current={isActive('/officer/audit-logs') ? 'page' : undefined}>
                      Audit Logs
                    </Link>
                  </>
                )}
                ...
                {user.role === 'admin' && (
                  <>
                    <Link to="/admin/dashboard" className={`nav-link ${isActive('/admin/dashboard') ? 'active' : ''}`} aria-current={isActive('/admin/dashboard') ? 'page' : undefined}>
                      Dashboard
                    </Link>
                    <Link to="/demand-map" className={`nav-link ${isActive('/demand-map') ? 'active' : ''}`} aria-current={isActive('/demand-map') ? 'page' : undefined}>
                      Demand Map
                    ...
                    <Link to="/admin/audit-logs" className={`nav-link ${isActive('/admin/audit-logs') ? 'active' : ''}`} aria-current={isActive('/admin/audit-logs') ? 'page' : undefined}>
                      Audit Logs
                    </Link>
                  </>
                )}
          ...
          <div className="nav-actions">
            {user ? (
              ...
              <>
                ...
                <Link to="/register" className={`btn btn-sm ${isActive('/register') ? 'active' : ''}`}>
                  Register
                </Link>
              </>
            )}
